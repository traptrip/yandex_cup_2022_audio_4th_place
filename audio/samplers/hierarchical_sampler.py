...
class HierarchicalSampler(BatchSampler):
    def __init__(
        ...
        # checks
        assert (
            self.batch_size % self.nb_categories == 0
        ), f"batch_size should be a multiple of {self.nb_categories}"
        ...
        self.super_pairs = list(
            itertools.combinations(set(dataset.super_labels), self.nb_categories)
        )
        self.reshuffle()
